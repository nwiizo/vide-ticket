...
impl FileStorage {
    ...
    fn ticket_path(&self, id: &TicketId) -> PathBuf {
        self.tickets_dir().join(format!("{}.yaml", id))
    }
    ...
    pub fn ensure_directories(&self) -> Result<()> {
        fs::create_dir_all(&self.tickets_dir())
            .context("Failed to create tickets directory")?;
        Ok(())
    }
    ...
    pub fn save_ticket(&self, ticket: &Ticket) -> Result<()> {
        ...
        fs::write(&path, yaml)
            .with_context(|| format!("Failed to write ticket to {:?}", path))?;
    ...
    pub fn load_ticket(&self, id: &TicketId) -> Result<Ticket> {
        ...
        if !path.exists() {
            return Err(VideTicketError::TicketNotFound {
                id: id.to_string(),
            });
        }
        ...
        let ticket: Ticket = serde_yaml::from_str(&yaml)
    ...
    pub fn delete_ticket(&self, id: &TicketId) -> Result<()> {
        ...
        fs::remove_file(&path)
            .with_context(|| format!("Failed to delete ticket at {:?}", path))?;
    ...
    pub fn get_active_ticket(&self) -> Result<Option<TicketId>> {
        ...
        let id = TicketId::parse_str(content.trim())
            .context("Failed to parse active ticket ID")?;
    ...
    pub fn clear_active_ticket(&self) -> Result<()> {
        ...
        if path.exists() {
            fs::remove_file(&path)
                .context("Failed to clear active ticket")?;
        }
...
impl FileStorage {
    ...
    pub fn load_state(&self) -> Result<ProjectState> {
        ...
        let state: ProjectState = serde_yaml::from_str(&yaml)
