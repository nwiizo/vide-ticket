...
pub fn handle_list_command(
    ...
    // Apply filters
    tickets = filter_tickets(tickets, status, priority, assignee, archived, since_date, until_date)?;
...
fn parse_date_filter(date_str: &str) -> Result<DateTime<Utc>> {
    ...
    if date_str == "today" {
        return Ok(Local::now().date_naive().and_hms_opt(0, 0, 0).unwrap().and_local_timezone(Local).unwrap().with_timezone(&Utc));
    } else if date_str == "yesterday" {
        return Ok((Local::now() - Duration::days(1)).date_naive().and_hms_opt(0, 0, 0).unwrap().and_local_timezone(Local).unwrap().with_timezone(&Utc));
        ...
        .captures(&date_str)
    {
        if let Some(days_str) = captures.get(1) {
            if let Ok(days) = days_str.as_str().parse::<i64>() {
                return Ok((Local::now() - Duration::days(days)).date_naive().and_hms_opt(0, 0, 0).unwrap().and_local_timezone(Local).unwrap().with_timezone(&Utc));
            }
        }
    }
        ...
        .captures(&date_str)
    {
        if let Some(weeks_str) = captures.get(1) {
            if let Ok(weeks) = weeks_str.as_str().parse::<i64>() {
                return Ok((Local::now() - Duration::weeks(weeks)).date_naive().and_hms_opt(0, 0, 0).unwrap().and_local_timezone(Local).unwrap().with_timezone(&Utc));
            }
        }
    }
...
fn filter_tickets(
    ...
    if let Some(priority_str) = priority {
        let priority = Priority::try_from(priority_str.as_str())
...
mod tests {
    ...
    fn test_parse_date_filter() {
        ...
        assert!(diff.num_hours() >= 24 && diff.num_hours() <= 48,
                "Expected yesterday to be 24-48 hours ago, got {} hours", diff.num_hours());
        ...
        assert!(diff.num_hours() >= 72 && diff.num_hours() <= 96,
                "Expected 3 days ago to be 72-96 hours ago, got {} hours", diff.num_hours());
