use crate::cli::{OutputFormatter, validate_slug, find_project_root};
...
pub fn handle_new_command(
    ...
    if storage.ticket_exists_with_slug(&slug)? {
        return Err(VideTicketError::DuplicateTicket {
            slug: slug.clone(),
        });
    }
...
mod tests {
    ...
    fn test_create_ticket_from_slug() {
        ...
        assert!(ticket.slug.ends_with("-fix-login-bug"), "Expected slug to end with '-fix-login-bug', got: {}", ticket.slug);
