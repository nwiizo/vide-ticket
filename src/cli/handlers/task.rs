...
pub fn handle_task_complete(
    ...
    if !task_found {
        return Err(VideTicketError::custom(format!("Task '{}' not found in ticket", task_id)));
    }
    ...
    if output.is_json() {
        ...
        output.info(&format!("Progress: {}/{} tasks completed", completed_count, total_count));
...
pub fn handle_task_list(
    ...
    if output.is_json() {
        ...
        output.info(&format!("Progress: {}/{} completed", completed_count, total_count));
...
pub fn handle_task_remove(
    ...
    // Find the task
    let task_index = ticket.tasks.iter().position(|t| t.id == task_id)
        .ok_or_else(|| VideTicketError::custom(format!("Task '{}' not found in ticket", task_id)))?;
    
    let task = &ticket.tasks[task_index];
    
    // Confirm removal if not forced
    if !force {
        output.warning(&format!("Are you sure you want to remove task: '{}'?", task.title));
