...
impl<S: DocumentStore> DocumentOperations<S> {
    ...
    pub fn new(store: S, base_dir: PathBuf) -> Self {
        Self { store, base_dir }
    }
    ...
    pub fn save_in_subdir<T: Serialize>(
        &self,
        id: &str,
        filename: &str,
        data: &T,
    ...
    pub fn load_from_subdir<T: DeserializeOwned>(
        &self,
        id: &str,
        filename: &str,
    ) -> Result<T> {
...
mod tests {
    ...
    fn test_document_operations() {
        ...
        ops.save_text_in_subdir("id2", "test.txt", "Hello, World!").unwrap();
