...
impl SpecManager {
    ...
    fn save_metadata(&self, metadata: &SpecMetadata) -> Result<()> {
        ...
        let content =
            serde_json::to_string_pretty(metadata).context("Failed to serialize spec metadata")?;
    ...
    fn load_document(&self, spec_dir: &Path, doc_type: SpecDocumentType) -> Result<Option<String>> {
        let doc_path = spec_dir.join(doc_type.file_name());
...
mod tests {
    ...
    fn test_spec_manager_creation() {
        ...
        assert_eq!(manager.specs_dir, specs_dir);
    }
